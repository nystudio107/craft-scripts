...
rsync -a -z -e "ssh -p ${REMOTE_SSH_PORT}" "${REMOTE_SSH_LOGIN}:${REMOTE_BACKUPS_PATH}${REMOTE_DB_NAME}" "${LOCAL_BACKUPS_PATH}" --progress
echo "*** Synced backups from ${REMOTE_BACKUPS_PATH}${REMOTE_DB_NAME}"
