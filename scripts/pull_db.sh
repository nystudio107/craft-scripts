...
ssh $REMOTE_SSH_LOGIN -p $REMOTE_SSH_PORT "mysqldump -u '$REMOTE_DB_USER' -p'$REMOTE_DB_PASSWORD' --host=$REMOTE_DB_HOST --port=$REMOTE_DB_PORT '$REMOTE_DB_NAME' > $TMP_DB_PATH"
